...
def main():
    try:
        post("/auth/register", {"username": "newbie", "password": "pass1234"})
    except requests.HTTPError:
        pass
    login = post("/auth/login", {"username": "newbie", "password": "pass1234"})
    headers = {"Authorization": f"Bearer {login['token']}"}
    session = post("/sessions", {"title": "no data test"}, headers)
    task = post(
        "/tasks",
