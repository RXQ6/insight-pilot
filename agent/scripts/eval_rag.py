"""RAG 检索质量评测：recall@k / hit@k。

用法（先 build_knowledge_base 建库）：
    .venv/Scripts/python scripts/eval_rag.py            # recall@5
    .venv/Scripts/python scripts/eval_rag.py --top-k 10

报告输出到 agent/data/rag_eval_report.json。
"""

import argparse
import json
import sys
import time
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from insight_agent.config import settings
from insight_agent.rag.retriever import retrieve_top_k

DATA_DIR = Path(__file__).resolve().parents[1] / "data"
CASES_PATH = DATA_DIR / "rag_eval_cases.json"
REPORT_PATH = DATA_DIR / "rag_eval_report.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="RAG recall@k evaluation")
    parser.add_argument("--top-k", type=int, default=5, help="retrieval depth")
    args = parser.parse_args()
    top_k = args.top_k

    cases = json.loads(CASES_PATH.read_text(encoding="utf-8"))
    results = []
    hits = 0
    for case in cases:
        started = time.time()
        chunks = retrieve_top_k(case["question"], top_k=top_k)
        elapsed_ms = int((time.time() - started) * 1000)
        hit = any(case["expected_title"] in chunk for chunk in chunks)
        hits += 1 if hit else 0
        results.append(
            {
                "id": case["id"],
                "question": case["question"],
                "expected_title": case["expected_title"],
                "hit": hit,
                "latencyMs": elapsed_ms,
                "retrieved_titles": sorted(
                    {chunk.split("|")[1].split("]")[0] if "|" in chunk else "?" for chunk in chunks}
                ),
            }
        )

    total = len(results)
    by_doc = {}
    for item in results:
        doc = by_doc.setdefault(item["expected_title"], {"total": 0, "hits": 0})
        doc["total"] += 1
        doc["hits"] += 1 if item["hit"] else 0

    report = {
        "runAt": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "mode": "hybrid (keyword+vector, RRF)" if settings.embedding_enabled else "keyword only",
        "topK": top_k,
        "total": total,
        "hits": hits,
        "recallAtK": round(hits / total, 4),
        "avgLatencyMs": round(sum(item["latencyMs"] for item in results) / total, 1),
        "byDoc": by_doc,
        "failed": [item["id"] for item in results if not item["hit"]],
        "details": results,
    }
    REPORT_PATH.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
