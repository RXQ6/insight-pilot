...
def main():
    ...
    session = post("/sessions", {"title": "dataset test"}, headers)
    task = post(
        "/tasks",
        {"sessionId": session["sessionId"], "message": "dataset_2_b1928d4f85 表中 amount 的总和是多少？"},
        headers,
    )
    status = None
    for _ in range(25):
    ...
    print(
        json.dumps(
            {"status": status["status"], "answerLen": len(answer), "has4500": "4500" in answer},
            ensure_ascii=True,
        )
    )
