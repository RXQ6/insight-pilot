...
3. **评测驱动迭代**：116 条评测集覆盖基础 SQL、多表关联、时间趋势、图表推荐、安全、追问及窗口函数/留存/同比/异常等进阶垂直场景；SQL 准确率从 48% 迭代到 74.7%，总通过率 54% → 78.4%，安全拦截率 100%，单次成本 ¥0.006，自动化报告 + 三端 CI；RAG 检索关键词+向量（bge-m3）混合召回 + RRF 融合，30 条分层标注（领域提问 recall@1 80% / recall@3 100%，口语化集 30% 暴露语义泛化短板）
