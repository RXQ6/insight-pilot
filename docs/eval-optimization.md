...
## 当前进度（2026-08-20 实测，116 条）

| 指标 | 基线（8-12） | 第一轮（8-19） | **第二轮 + ReAct（8-20）** |
|---|---|---|---|
| 总通过率 | 54% | 67.2%（78/116） | **78.4%（91/116）** |
| SQL 准确率 | 48% | 62.7% | **74.7%** |
| 图表可用率 | 30% | 60% | **80%** |
| 安全拦截率 | 100% | 100% | 100% |
| sql_join | 8/25 | 15/25 | **19/25** |
| time_trend | 3/20 | 7/20 | **12/20** |
| anomaly | — | 3/4 | **4/4** |

第二轮优化（time_trend/yoy_mom/join 定向 few-shot + ReAct 决策循环）已生效：SQL 准确率 62.7% → 74.7%。剩余薄弱点：yoy_mom（2/4）、sql_join 复杂子查询（007/009/013/018/020/023）、sql_single 5 条 easy 持久失败（019/021/022 疑似判定或生成细节，可用 diagnose_case.py 排查）。
...
| 第二轮（已达成） | **74.7%** | 定向 few-shot + ReAct 工具循环 |
| 进阶场景 | 单独统计 | 窗口 3/4、留存 3/4、异常 4/4；同比环比 2/4 待攻 |

## 面试怎么讲

- "我建了 116 条评测集，覆盖基础 SQL、多表关联、时间趋势、图表推荐、安全、追问澄清，以及窗口函数、留存、同比环比、异常检测等进阶垂直场景"
- "SQL 准确率从 48% 迭代到 74.7%（每次改动跑评测、留报告、用失败用例反哺 prompt）"
- "安全拦截率 100%，单次成本 ¥0.006"——数字要能背出来
