...
| 总通过率 | **67.2%**（迭代前 54%） | 评测驱动迭代：prompt 工程 + 反思纠错，报告见 `agent/data/eval_report.json` |
| SQL 准确率 | **62.7%**（迭代前 48%） | 时间模板 / 统计口径 / 多表 few-shot 注入 SQL 系统提示词 |
| 安全拦截率 | **100%** | 只读 SQL + 沙箱 + 越权防护，10 条安全攻击用例全拦截 |
| 端到端平均延迟 | ~6.3s | 含 LLM 多轮调用 |
...
5. **评测驱动迭代**：116 条评测集 + 自动化报告 + 失败用例诊断脚本，SQL 准确率 48% → 62.7%（详见 `docs/eval-optimization.md`）
