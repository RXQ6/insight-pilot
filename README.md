...
**谁在用**：业务/运营人员需要随时查业务数据，但没有 SQL 能力。

**原来怎么解决**：
- 提需求给数据团队 → 排期（小时/天级）
- 手动导 Excel + 透视表 → 操作繁琐、口径不统一
- BI 报表固定字段 → 临时问题答不了

**Agent 介入后**：
- **自然语言直达数据**：问一句就能查，免提需求排期
- **分钟级出结果**：单任务端到端 ~7s，对比原排期是数量级提升
- **口径统一**：知识库（数据字典/报表口径/查询约定）+ 评测约束，减少"同一指标各说各话"
- **可验证**：116 条评测 SQL 准确率 74.7%、安全拦截 100%，每任务成本与延迟可见
...
- 工具层：只读 SQL、Python 沙箱、ECharts 图表、知识库 RAG（关键词+向量混合检索 + RRF 融合，30 条分层标注：领域提问 recall@1 80% / recall@3 100%）、CSV 结果导出
